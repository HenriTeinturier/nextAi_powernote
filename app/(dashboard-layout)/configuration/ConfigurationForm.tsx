...
import { updateConfigurations } from "./configuration.action";
...
export const ConfigurationForm = (props: ConfigurationFormProps) => {
  ...
  const handleSubmit = useMutation({
    mutationFn: async (values: ConfigurationFormType[]) => {
      const { data, serverError } = await updateConfigurations(values);
      if (!data || serverError) {
        toast.error(serverError ?? "An error occured");
        return;
      }
                ...
                onClick={() => {
                  enqueueDialog({
                    ...
                    action: {
                      label: "Delete",
                      onClick: () => {
                        const filteredConfiguration = form
